...
impl SmoothScroll {
    ...
    pub fn show(
        ...
        if let Some(sp) = self.scroll_offset_x {
            self.char_scroll_positon.x = sp.floor();
        }
        if let Some(sp) = self.scroll_offset_y {
            self.char_scroll_positon.y = sp.floor();
        }
    ...
    fn show_vertical_scrollbar(
        ...
        let bar_top = calc.terminal_rect.top()
            + calc.terminal_rect.height() * self.char_scroll_positon.y
    ...
    fn show_horizontal_scrollbar(
        ...
        let bar_left = calc.terminal_rect.left()
            + calc.terminal_rect.width() * self.char_scroll_positon.x
        ...
        if response.hovered() {
            ...
            for e in events {
                if let egui::Event::Scroll(vec) = e {
                    self.char_scroll_positon.y -= vec.y;
                    self.set_scroll_positon = true;
                }
            }
    ...
    fn handle_user_input_horiz(
        ...
        if response.clicked() {
            if let Some(mouse_pos) = response.interact_pointer_pos() {
                if mouse_pos.y > y {
                    ...
                    self.char_scroll_positon = Vec2::new(
                        calc.char_width * calc.font_width * (mx - bg_rect.left()) / bg_rect.width(),
                        self.char_scroll_positon.y,
                    );
                    self.set_scroll_positon = true;
                }
            }
        ...
        if response.hovered() {
            ...
            for e in events {
                if let egui::Event::Scroll(vec) = e {
                    self.char_scroll_positon.x -= vec.x;
                    self.set_scroll_positon = true;
                }
            }
