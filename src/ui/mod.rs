...
pub fn show_terminal_area(
    ...
    SmoothScroll::new().with_lock_focus(focus_lock).show(
        ui,
        |rect| {
            let size = rect.size();
        ...
        |ui, rect, scrollbar_rect| {
            let viewport_top = scrollbar_rect.char_scroll_positon * scrollbar_rect.scale.y;
            scrollbar_rect.first_line = viewport_top / scrollbar_rect.char_size.y;

            {
                let buffer_view = &mut buffer_view.lock();
                buffer_view.char_size = scrollbar_rect.char_size;
            ...
            let buffer_rect = scrollbar_rect.buffer_rect;
